...
test.describe('[UI] QE-910: DPP Group IIG and NB Category Options Validation', { tag: ['@smoke', '@regression', '@core-ask'] }, () => {
  let loginPage;
  let coreAskPage;

  test('[TC-1102] Verify IIG and NB category options are displayed in DPP Group dropdown', async ({ page }) => {
    ...
    // Step 5: Verify IIG and NB options
    const options = await coreAskPage.getDppGroupOptions();
    TD.dppGroups.iigAndNb.forEach(option => {
