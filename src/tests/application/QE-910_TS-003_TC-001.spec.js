...
test.describe('[UI] QE-910 TS-003: Verify Outgoing Resource field behavior for Replacement reason', { tag: ['@regression', '@kpmg'] }, () => {
  let coreAskPage;

  test('[QE-910 TS-003 TC-001] Verify Outgoing Resource field is editable textbox when Reason is Replacement', async ({ page }) => {
    ...
    // Step 5: Select 'Replacement' from the Reason for DPP Need dropdown
    ...
    expect(selectedReason).toBe(TD.reasonForNeed.replacement);
    ...
    await coreAskPage.fillOutgoingResource(TD.outgoingResource.sampleReplacement);
    ...
    expect(enteredValue).toBe(TD.outgoingResource.sampleReplacement);
  });
});