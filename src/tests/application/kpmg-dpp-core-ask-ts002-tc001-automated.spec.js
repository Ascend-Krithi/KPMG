...
test.describe('[UI] QE-910 TS-002: Outgoing Resource Field Verification for Addition', { tag: ['@regression', '@kpmg-dpp'] }, () => {
  let coreAskPage;

  test('[QE-910 TS-002 TC-001] Verify Outgoing Resource field is N/A and read-only when Reason for DPP Need = Addition', async ({ page }) => {
    ...
    // Step 4 & 5: Select 'Addition' from Reason for DPP Need dropdown
    const reasonOptions = await coreAskPage.getReasonForDppNeedOptions();
    expect(reasonOptions).toContain(TD.reasonForDppNeedOptions.addition);
    expect(reasonOptions).toContain(TD.reasonForDppNeedOptions.replacement);
    expect(reasonOptions).toContain(TD.reasonForDppNeedOptions.succession);
    
    await coreAskPage.selectReasonForDppNeed(TD.reasonForDppNeedOptions.addition);
    
    // Step 6: Verify Outgoing Resource field value is N/A
    const outgoingResourceValue = await coreAskPage.getOutgoingResourceValue();
    expect(outgoingResourceValue).toBe(TD.naValue);
