...
test.describe('[UI] QE-911 TS-001: Core ASK Review Form Display - NBL Role', { tag: ['@smoke', '@regression', '@caw-core-ask'] }, () => {
  let cawPage;

  test('[QE-911 TS-001 TC-001] Verify NBL role can view CAW_FRM_CoreAskReview form in read-only mode', async ({ page }) => {
    ...
    const submitterDetails = await cawPage.getSubmitterDetails();
    expect(submitterDetails).toBeTruthy();
    
    const isBuGridVisible = await cawPage.isBuPlanningGridVisible();
    expect(isBuGridVisible).toBeTruthy();
    
    const fiscalYear = await cawPage.getFiscalYear();
