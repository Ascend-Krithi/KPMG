...
test.describe('[UI] QE-910: DPP Group ASG Category Options Validation', { tag: ['@smoke', '@regression', '@core-ask'] }, () => {
  let loginPage;
  let coreAskPage;

  test('[TC-1100] Verify ASG category options are displayed in DPP Group dropdown', async ({ page }) => {
    ...
    // Step 5: Verify ASG options
    const options = await coreAskPage.getDppGroupOptions();
    TD.dppGroups.asg.forEach(asgOption => {
      expect(options).toContain(asgOption);
    });
  });
});