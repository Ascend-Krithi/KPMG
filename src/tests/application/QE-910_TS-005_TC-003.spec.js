...
test.describe('[UI] QE-910 TS-005: Verify Role Posting field is hidden for Managing Director level', { tag: ['@regression', '@kpmg'] }, () => {
  let coreAskPage;

  test('[QE-910 TS-005 TC-003] Verify Role Posting field is not visible when Level Needed is Managing Director', async ({ page }) => {
    ...
    // Step 4: Locate Level Needed dropdown and select 'Managing Director'
    ...
    expect(selectedLevel).toBe(TD.levelNeeded.managingDirector);
