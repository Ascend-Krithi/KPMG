...
test.describe('[UI] QE-910 TS-007: DPP Leadership Role - Route To Options', { tag: ['@regression', '@kpmg-dpp'] }, () => {
  let loginPage;
  let dashboardPage;
  let coreAskPage;

  test('[QE-910 TS-007 TC-001] Verify DPP Leadership user can only route to DPP Ops', async ({ page }) => {
    ...
    await loginPage.login(TD.credentials.dppLeadershipUser.username, TD.credentials.dppLeadershipUser.password);
    ...
    expect(options).toContain(TD.coreAsk.routeTo.dppOps);

    // Step 6: Select DPP Ops
    await coreAskPage.selectRouteTo(TD.coreAsk.routeTo.dppOps);
